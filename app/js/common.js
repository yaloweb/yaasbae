$(function () {
  ...
  $('.product-image-thumb').on('click', function () {
    ...
    if ( $(this).hasClass('video-thumb') ) {
      let currOwlItem = prodImgGallery.find(`.owl-item:eq(${i})`),
          videoFrame = currOwlItem.find('video');
      videoFrame[0].play()
          // videoFrame = currOwlItem.find('iframe'),
      //     videoSrc = videoFrame.attr('src').split('?')[0],
      //     videoId = videoSrc.split('').reverse().join('').split('/')[0].split('').reverse().join('');
      // videoFrame
      //   .attr('src', `${videoSrc}?loop=1&playlist=${videoId}&mute=1&autoplay=1`)
      //   .attr('allow', 'autoplay');
    }
    else {
      prodImgGallery.find('video').each(function() {
        $(this)[0].pause()
      });
    }
  });
  ...
  prodImgGallery.on('translate.owl.carousel', function (e) {
    ...
    if ( videoFrame.length != 0 ) {
      videoFrame[0].play()
      // let videoSrc = videoFrame.attr('src').split('?')[0],
      //     videoId = videoSrc.split('').reverse().join('').split('/')[0].split('').reverse().join('');
      // videoFrame
      //   .attr('src', `${videoSrc}?loop=1&playlist=${videoId}&mute=1&autoplay=1`)
      //   .attr('allow', 'autoplay');
